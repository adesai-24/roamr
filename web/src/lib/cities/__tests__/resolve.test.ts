 ...
 * (provider, provider_place_id) exactly like the unique constraint, so a second
 * write of the same place returns the row the first one created instead of a
 * new id.
 */
function createFakeDatabase() {
  ...
  const from = vi.fn((table: string) => {
    ...
    return {
      insert,
      upsert(payload: Payload, options?: { onConflict?: string }) {
        upsertCalls.push({ payload, options });
        const existing = rows.get(key(payload));
        const merged = {
          ...(existing ?? { id: `city-${nextId++}`, created_at: "2026-01-03T00:00:00.000Z" }),
          ...payload,
        } as CityRow;
        rows.set(key(payload), merged);
        return {
  ...
  return { client: { from }, from, insert, upsertCalls, rows, selectFilters };
}

function useAdmin(db: ReturnType<typeof createFakeDatabase>) {
...
describe("resolveCity", () => {
  it("upserts onto the unique constraint that backs the dedupe", async () => {
    ...
    expect(db.upsertCalls[0]?.options).toEqual({ onConflict: "provider,provider_place_id" });
